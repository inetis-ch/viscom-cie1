...
Dans le domaine du développement, "réactif" signifie "sensible". La programmation du site web est capable de reconnaître quel navigateur, quel système d'exploitation et/ou quelle résolution d'affichage est utilisé par l'utilisateur et peut y réagir. Concrètement, cela signifie que différents affichages sont possibles en fonction des détails de l'appareil détecté.

Une étape importante dans la conception et la réalisation d'un site web réactif est la planification: comment le site et sa mise en page doivent-ils se comporter de manière réactive ?
...
Les points d'arrêt vous permettent de modifier la mise en page lorsque la taille de l'écran dépasse un certain point. Par exemple, trois colonnes sur un bureau peuvent se décomposer en une seule colonne sur un smartphone. L'endroit où vous devez fixer ces points d'arrêt (breakpoint) dépend de votre contenu.
...
Pour de simples changements de mise en page, vous n'avez parfois pas besoin d'interroger les "media queries". Avec [Flexbox](/guide/09_css_layout), par exemple, vous pouvez faire en sorte que les éléments passent automatiquement sur deux lignes si le contenu ne permets pas de s'afficher sur une seule.
