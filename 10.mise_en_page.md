...
Chaque navigateur interprète la balise `<span>` avec `display : inline;` par défaut, sans que vous ayez à définir quoi que ce soit d'autre dans le CSS. Un élément HTML de style "display : inline" peut faire partie du corps du texte et peut s'étendre sur plusieurs lignes. Les espaces avec "padding" ou "marge" (comme dans le modèle de la boîte) ne sont pas acceptés par "display : inline;`.

    span {
      frontière : 0,1rem de tomate solide ;
    }
...
Je suis un texte dont <span>une petite partie est mise en évidence et dont le contenu peut dépasser plusieurs lignes.
...
Le "bloc en ligne" combine le comportement du "bloc en ligne" et du "bloc". Contrairement à "en ligne", le "bloc en ligne" peut également comprendre les propriétés de "rembourrage" et de "marge".
...
Display : flex" décrit Flexbox, qui fournit un moyen avancé de concevoir des mises en page modernes et réactives sans avoir à utiliser des propriétés comme "position", "float" ou "clear".

### Quand Flexbox et quand Grid ?

Difference Flexbox to Grid](/assets/img/grid-vs-flexbox.3e4cb859.png) [source](https://ishadeed.com/article/grid-layout-flexbox-components/)
...
Concepts de la grille CSS](/assets/img/cssGrid.a5e3f09f.png) [source](https://wiki.selfhtml.org/wiki/CSS/Tutorials/Grid/Einführung)
